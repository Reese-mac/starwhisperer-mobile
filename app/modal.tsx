...
import { SafeAreaView, View, Text, StyleSheet, TouchableOpacity, Image, ImageBackground, ScrollView, Modal } from 'react-native';
...
const ModalScreen = () => {
  ...
  const nasaMoonImage = 'https://svs.gsfc.nasa.gov/vis/a000000/a005400/a005415/frames/730x730_1x1_30p/moon.8558.jpg';
  const fallbackImage = nasaMoonImage;
  ...
  const handleOpenCard = (item: MoonPhaseEntry) => {
    ...
    setSelectedImageUri(phaseImages[item.name] ?? fallbackImage);
  };

  const renderDetailModal = () => {
    if (!selectedCard) return null;
    const imageUri = selectedImageUri ?? phaseImages[selectedCard.name] ?? fallbackImage;
    const illumFromValue = selectedCard.phaseValue !== undefined ? selectedCard.phaseValue : undefined;
    const illumFromText = parseFloat((selectedCard.illumination || '0').replace('%', '')) / 100;
    return (
        ...
        <View style={styles.detailOverlay}>
          ...
          <View style={styles.detailCard}>
            <ImageBackground
              ...
              resizeMode="contain"
              onError={() => setSelectedImageUri(fallbackImage)}
            />
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        >
          <Ionicons name="chevron-back" size={22} color="#f1f1ff" />
        </TouchableOpacity>
        ...
        <ScrollView contentContainerStyle={{ paddingTop: 80, paddingBottom: 40 }}>
          ...
          <View style={styles.collectionPanel}>
            ...
            {loadingPhases ? (
              ...
              phaseCards.map(item => (
                <View key={`${item.name}-${item.riseTime}`} style={styles.cardRow}>
                  <View style={{ flex: 1 }}>
                  ...
                  <TouchableOpacity onPress={() => handleOpenCard(item)} activeOpacity={0.85}>
